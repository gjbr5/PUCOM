...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    editMode: function (data, noreset, skipField, skipCardIndex) {
        if (document.get.offline_forms == 'true' && document.get.puppet == '1') {
            // We are already adding edit.submission.js file as static must-have file on puppet side.
            // don't make any additional requests, while preparing our offline resources with puppeteer, 
            return;
        }
        var preLink = "";
        if (!JotForm.debug) {
        ...
        if(document.get.offline_forms == 'true' && JotForm.isJotFormNext) {
        ...
        $H(prices).each(function (pair) {
            ...
            if ($(pair.key) && $(pair.key).checked) {
                ...
                if (pair.value.tax) {
                    ...
                    taxAmountWithoutDiscount = parsePriceWithoutComma(priceWithoutDiscount * (taxRate / 100));
                }
